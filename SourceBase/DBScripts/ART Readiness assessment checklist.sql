...
SELECT 'ART Readiness Assessment Checklist' [FormName],a.ptn_pk,(convert(varchar(50), decryptbykey(a.FirstName))+' '+ convert(varchar(50), decryptbykey(a.MiddleName)) + ' '+ convert(varchar(50), decryptbykey(a.LastName))) Name,
...
GO
--insert ART Redainess Feature 
IF NOT EXISTS(Select * FROM mst_Feature WHERE FeatureName = 'ART Readiness Assessment Checklist')
BEGIN
	...
	('ART Readiness Assessment Checklist'
	, 0
	, 0
	, 0
	, 1
...
--mst_visittype
IF NOT EXISTS(SELECT * FROM mst_VisitType WHERE VisitName = 'ART Readiness Assessment Checklist')
BEGIN
	...
	, [SystemId]
	, [FeatureID]) 
	VALUES ( 'ART Readiness Assessment Checklist'
	, 0
	, 1
	, GETDATE()	
	, 1
...
ALTER PROCEDURE [dbo].[pr_KNHPMTCTART_SaveData] 
	@patientID INT,
	@LocationID INT,
	@Visit_ID INT,
	@UnderstandHiv Varchar(50),
...
BEGIN
 ...
 values(@patientID,@LocationId,@visitdate,229,@DataQlty,0,1,getdate())
