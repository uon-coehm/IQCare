    ...
    public class BControls : ProcessBase, IControl
    {
        ...
        public DataSet GetControl()
        {
            lock (this)
            {
                ...
                return (DataSet)ControlManager.ReturnObject(oUtility.theParams, "pr_Admin_SelectControl_Constella", ClsUtility.ObjectEnum.DataSet);
            }
        }
        ...
        public int SaveNewControl(string Name, string sDataType,int UserID)
        {
            try
            {
                ...
                oUtility.AddParameters("@DataType", SqlDbType.VarChar, sDataType);
                oUtility.AddParameters("@UserId", SqlDbType.Int, UserID.ToString());

                DataRow theDR;
                theDR = (DataRow)ControlManager.ReturnObject(oUtility.theParams, "Pr_Admin_AddControl_Constella", ClsUtility.ObjectEnum.DataRow);
        ...
        public int UpdateControl(int ControlId,string Name, string sDataType, int UserID)
        {
            ...
            try
            {
                ...
                DataRow theDR;
                theDR = (DataRow)ControlManager.ReturnObject(oUtility.theParams, "Pr_Admin_UpdateControl_Constella", ClsUtility.ObjectEnum.DataRow);
                if (Convert.ToInt32(theDR[0]) == 0)
                {
                    MsgBuilder theBL = new MsgBuilder();
                    theBL.DataElements["MessageText"] = "Error in Saving Control record. Try Again..";
