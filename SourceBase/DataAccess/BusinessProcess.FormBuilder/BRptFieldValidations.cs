     ...
     public class BRptFieldValidations : ProcessBase, IRptFieldValidations
    {
         ...
         public DataSet GetRptFieldDetails()
        {
            lock (this)
            {
                ...
                return (DataSet)RptField.ReturnObject(oUtility.theParams, "Pr_FormBuilder_GetCareEndedForm_Futures", ClsUtility.ObjectEnum.DataSet);
            }
        }
         ...
         public DataTable ParseXml(string stringxml,int FacilityId,int ModuleId)
         {
             lock (this)
             {
                 ClsObject theQB = new ClsObject();
                 oUtility.Init_Hashtable();
                 oUtility.AddParameters("@data", SqlDbType.Xml, stringxml);
                 oUtility.AddParameters("@FaciltyId", SqlDbType.Int, FacilityId.ToString());
                 oUtility.AddParameters("@SelectModuleId", SqlDbType.Int, ModuleId.ToString());
                 return (DataTable)theQB.ReturnObject(oUtility.theParams, "pr_General_ProcessXml", ClsUtility.ObjectEnum.DataTable);
             }
         }
