    ...
    public class BMigration: ProcessBase,IMigration 
    {
        ...
        public int UpsizeData(DataTable theDT,DataTable tblDataTypes, string DBName,string TableName)
        {
            ...
            int i = 0;
                ...
                try
                {
                    ...
                    int RowsAffected = (Int32)objUpsize.ReturnObject(oUtility.theParams, cmd, ClsUtility.ObjectEnum.ExecuteNonQuery);
                    ...
                    foreach (DataRow theDR in theDT.Rows)
                    {
                        ...
                        cmd = cmd + theColumns + ")";
                        oUtility.Init_Hashtable();
                        RowsAffected = (Int32)objUpsize.ReturnObject(oUtility.theParams, cmd, ClsUtility.ObjectEnum.ExecuteNonQuery);
                    }
                    //////theTran.Commit();
                    //////theCon.Close();
        ...
        public int MigrateData(string cmd, int Location, string DBName, int CommandId)
        {
            ...
            try
            {
                ...
                oUtility.Init_Hashtable();
                oUtility.AddParameters("@dbname", SqlDbType.VarChar, DBName);
                if (Location > 0)
                    oUtility.AddParameters("@Loc", SqlDbType.Int, Location.ToString());
                int RowsEffected = (int)objMigration.ReturnObject(oUtility.theParams, cmd, ClsUtility.ObjectEnum.ExecuteNonQuery);

                string thecmd = "update migration set complete = 1 where id = " + CommandId;
                oUtility.Init_Hashtable();
                int rowseffected = (int)objMigration.ReturnObject(oUtility.theParams, thecmd, ClsUtility.ObjectEnum.ExecuteNonQuery);
                //////theTran.Commit();
                //////theCon.Close();
                DataMgr.CommitTransaction(this.Transaction);
                DataMgr.ReleaseConnection(this.Connection);
        ...
        public DataSet GetProcedures(string Constr,string DBName)
        {
            lock (this)
            {
                try
                {
                    ...
                    //////oUtility.Init_Hashtable();
                    //////oUtility.AddParameters("@dbname", SqlDbType.VarChar, DBName);
                    //////return (DataSet)theObject.ReturnObject(Constr, oUtility.theParams, "pr_GetProcedureList", ClsUtility.ObjectEnum.DataSet);
                    ...
                    oUtility.Init_Hashtable();
                    oUtility.AddParameters("@dbname", SqlDbType.VarChar, DBName);
                    return (DataSet)objProcList.ReturnObject(oUtility.theParams, "pr_GetProcedureList", ClsUtility.ObjectEnum.DataSet);
                }
                catch
                {
