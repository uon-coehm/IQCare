...
namespace IQCare.SCM
{
    public partial class frmPatientDrugDispense : Form
    {
        ...
        private DataTable CreatePharmacyDispenseTable()
        {
            ...
            theDT.Columns.Add("QtyUnitDisp", Type.GetType("System.Int32"));
            theDT.Columns.Add("Syrup", Type.GetType("System.Int32"));
            //
        ...
        private void BindPharmacyDispenseGrid(DataTable theDT)
        {
            try
            {
                ...
                DataGridViewTextBoxColumn theCol25 = new DataGridViewTextBoxColumn();
                theCol25.HeaderText = "QtyUnitDisp";
                theCol25.DataPropertyName = "QtyUnitDisp";
                theCol25.Name = "QtyUnitDisp";
                theCol25.Width = 10;
                theCol25.Visible = false;
                theCol25.ReadOnly = true;

                DataGridViewTextBoxColumn theCol26 = new DataGridViewTextBoxColumn();
                theCol26.HeaderText = "syrup";
                theCol26.DataPropertyName = "syrup";
                theCol26.Name = "syrup";
                theCol26.Width = 10;
                theCol26.Visible = false;
                theCol26.ReadOnly = true;
                ...
                grdDrugDispense.Columns.Add(theCol25);
                grdDrugDispense.Columns.Add(theCol26);
                //end
        ...
        private void lstSearch_DoubleClick(object sender, EventArgs e)
        {
            if (lstSearch.SelectedValue != "")
            {
                ...
                QtyUnitDisp = theDV[0]["QtyUnitDisp"].ToString();
                syrup = theDV[0]["syrup"].ToString();
                ...
                else
                {
                    ...
                    if ((theDispCurrentRow > -1) && (theDT.Rows.Count > 0))
                    {
                        ...
                        if (theDV.Count > 0)
                        {
                            ...
                            theDV[0]["QtyUnitDisp"] = QtyUnitDisp;
                            theDV[0]["syrup"] = syrup;
            ...
            try
            {
                ...
                if (tabDispense.TabPages[1].Focus() == true)
                {
                    ...
                    if (btnART.Enabled)
                    {
                        ...
                        //}
                        if (cmbprogram.SelectedValue.ToString() == "0" || cmbRegimenLine.SelectedValue.ToString() == "0" || cmbProvider.SelectedValue.ToString() == "0")
                        {
        ...
        private void grdDrugDispense_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 11 || e.ColumnIndex == 9 || e.ColumnIndex == 7)
            {
                if (grdDrugDispense.Rows[e.RowIndex].Cells["BatchNo"].Value.ToString() != "")
                {
                    ...
                    {
                        //if (grdDrugDispense.Rows[e.RowIndex].Cells["QtyUnitDisp"].Value.ToString() == "" || grdDrugDispense.Rows[e.RowIndex].Cells["QtyUnitDisp"].Value.ToString() == "0" || grdDrugDispense.Rows[e.RowIndex].Cells["QtyUnitDisp"].Value == DBNull.Value)
                        if (grdDrugDispense.Rows[e.RowIndex].Cells["syrup"].Value != null && grdDrugDispense.Rows[e.RowIndex].Cells["syrup"].Value.ToString() != "0")
                        {
                            if (grdDrugDispense.Rows[e.RowIndex].Cells["syrup"].Value.ToString() == "1")
                            {
                                //john syrups
                                grdDrugDispense.Rows[e.RowIndex].Cells["QtyPres"].Value = (Int32)Math.Ceiling(((Convert.ToDouble(grdDrugDispense.Rows[e.RowIndex].Cells["Dose"].Value.ToString()) * Convert.ToDouble(grdDrugDispense.Rows[e.RowIndex].Cells["freqMultiplier"].Value.ToString()) * Convert.ToDouble(grdDrugDispense.Rows[e.RowIndex].Cells["Duration"].Value.ToString())) / Convert.ToDouble(grdDrugDispense.Rows[e.RowIndex].Cells["QtyUnitDisp"].Value.ToString())));
                                //
                            }
                        }
                        else
        ...
        private void txtNoOfDays_TextChanged(object sender, EventArgs e)
        {
            if (txtNoOfDays.Text != "")
            {
                dtRefillApp.Value = DateTime.Today;
                dtRefillApp.Value = dtRefillApp.Value.AddDays(Convert.ToDouble(txtNoOfDays.Text));
            }
            else
            {
                dtRefillApp.Value = DateTime.Today;
        ...
        {
            if (dtRefillApp.Value.DayOfWeek.ToString() == "Saturday")
            {
                MessageBox.Show("The Selected day is a Saturday");
            }

            if (dtRefillApp.Value.DayOfWeek.ToString() == "Sunday")
            {
                MessageBox.Show("The Selected day is a Sunday");
            }
        }
        //john end
    }
