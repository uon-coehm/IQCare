...
namespace Touch.Custom_Forms
{
    public partial class frmVisitTouch : TouchUserControlBase
    {
        ...
        private void Init_Form()
        {
            ...
            DataTable dtDiffart = GetDataFromQuery("select artstartdate from mst_patient where ptn_pk =" + Convert.ToInt32(Request.QueryString["patientId"].ToString()));
            ...
            //check data diff
            DataTable dtDiff = GetDataFromQuery("select updatedate from dtl_regimenmap where ptn_pk =" + Convert.ToInt32(Request.QueryString["patientId"].ToString()));
            ...
            //set change and stop regimen reasons
            DataTable DTregreasons = GetDataFromQuery("select * from mst_reason where  categoryID=5 and systemID = 3");
            theBind.BindCombo(rcbARVChangeReason, DTregreasons, "Name", "ID");
            theBind.BindCombo(cbARVStopReason, DTregreasons, "Name", "ID");
        ...
        protected void SetFormVals()
        {
            ...
            if (ds.Tables.Count > 0)
            {
                ...
                if (dtCS.Rows.Count > 0)
                {
                    ...
                    txtTemp.Text = dtCS.Rows[0]["Temp"].ToString();
                    txtWeight.Text = dtCS.Rows[0]["Weight"].ToString();
                    txtHeight.Text = dtCS.Rows[0]["Height"].ToString();
                    txtRespRate.Text = dtCS.Rows[0]["RR"].ToString();
                    txtPulse.Text = dtCS.Rows[0]["HR"].ToString();
                    txtSystolic.Text = dtCS.Rows[0]["BPSystolic"].ToString();
                    txtDiastolic.Text = dtCS.Rows[0]["BPDiastolic"].ToString();
                    txtHeadCirc.Text = dtCS.Rows[0]["Headcircumference"].ToString();
                    txtMUAC.Text = dtCS.Rows[0]["MUAC"].ToString();
                    GetBMI();
                }
                ...
                if (dtTB.Rows.Count > 0)
                {
                    ...
                    {
                        if (item.Value.Contains(dtTB.Rows[0]["ContactTBTreatmentRcvd"].ToString()))
                ...
                if (dtTBS4.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow item in dtTBS4.Rows)
                    {
                        ...
                        else if (item["IsPatient"].ToString() == "1")
                        {
                            ...
                            {
                                RadButton YNVal = (RadButton)grItem.FindControl("btnResSens");
                                if (YNVal != null)
                                {
                                    if (grItem["ID"].Text == item["Drug"].ToString())
                                    {
                                        ...
                                        if (item["Resistance"].ToString() == "1")
                                            YNVal.SelectedToggleStateIndex = 2;
                ...
                //if (dtAEget.Rows.Count > 0)
                //{
                //    foreach (DataRow item in dtAEget.Rows)
                //    {
                ...
                //            {
                //                if (node.Value == item["AdverseEventID"].ToString())
                //                {
                //                    node.Checked = true;
                //                }
                //            }
        ...
        private void SaveVisit()
        {
            ...
            catch (Exception e)
            {
                ...
                ErrManager.LogError("Visit Page", e.Message, ErrorType.Error);
                IsError = true;
            }
            finally
            {
        ...
        private void SetSavedVals(ref objVisit tvs)
        {
            ...
            if (btnSexActiveYes.Checked)
            {
                ...
                if (cbOtherFamilyPlanning.SelectedItem.Text == "Select")
            ...
            if (btnAEYes.Checked)
            {
                ...
                foreach (GridDataItem item in rgAE.Items)
                {
                    ...
                    if (item["EventName"].Text.Contains("Other"))
            ...
            rt.RefferredID = _CheckInt(cbReferredTo.SelectedValue);
        ...
        private void OnTBsensNewClose()
        {
            ...
            {
                RadButton YNVal = (RadButton)item.FindControl("btnResSens");
                if (YNVal.SelectedToggleStateIndex != 0)
                {
                    object[] findVals = new object[1];
                    findVals[0] = item["ID"].Text;
                    ...
                    theDR["Type"] = YNVal.SelectedToggleState.Text;
